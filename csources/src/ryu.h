...
#define get_thread_t(x) (x->thread)
...
extern char * ryumalloc(struct order *order,size_t size);
...
extern int pse_stream_end_read(pse_stream *stream);
extern int pse_stream_end_write(pse_stream *stream);
extern char ** str_to_args(char *arg);
