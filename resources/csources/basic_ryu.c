...
void pse_stream_end_write(pse_stream *stream){
    if(stream==NULL) return;
    stream->writing_done = stream->writing_done | 0x0f;
    return;
}
void pse_stream_end_read(pse_stream *stream){
    if(stream==NULL) return;
    stream->writing_done = stream->writing_done | 0xf0;
}
int Is_st_reading(pse_stream *stream){
    if(stream==NULL) return 0;
    if(!(stream->writing_done & 0xf0)){
        /*Reading but could not have any data that can be read*/
        if( stream->writing_done & 0x0f &&  /*Is stream no longer be written*/
            stream->appendIndex <= stream->readIndex && /*Is there any bytes that can be read*/
            stream->appendBlock <= stream->readBlock    /*same as above*/
        ){
                    /*No data that can be read*/
...
pse_stream * get_read_stream(struct order *p,int th){return get_read_Stream(p,th);}
...
pse_stream * get_read_stream_back(struct order *p,int th){return get_read_Stream_back(p,th);}
...
pse_stream * get_write_stream(struct order *p,int th){return get_write_Stream(p,th);}
...
pse_stream * get_write_stream_back(struct order *p,int th){return get_write_Stream_back(p,th);}
...
char * read_line(struct order *order,pse_stream *stream){
	if(stream==NULL||!Is_st_reading(stream)){
		return NULL;
	}
    int size = 128;
	char *buff;
	...
	while((c=read_pse_stream(stream))!=EOF){
		...
		if(cou>=size){
			if(order==NULL){
                ...
                buff = ryurealloc(order,buff,size*=2);
            }
            if(buff == NULL){
                return NULL;
            }
